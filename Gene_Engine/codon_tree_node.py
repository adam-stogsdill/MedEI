...
class Node(object):
    ...
    def get_node_and_existence(self, nucleotide):
        for x in self.nodes:
            if x.value == nucleotide:
                return True, x
        return False, None

    def __getitem__(self, nucleotide):
        for x in self.nodes:
            if x.value == nucleotide:
                return x
        return None
...
def create_traversal_path(initial_node, path, final_value):
    if len(path) == 0:
        ...
        contains_nucl, first_node = initial_node.get_node_and_existence(path[0])
...
def path_traversal(initial_node, path):
    if initial_node.codon != "":
        return initial_node.codon
    node = initial_node[path[0]]
